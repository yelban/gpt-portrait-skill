...
> SKILL.md §3 / §21 / §29 / §30 的延伸。需要詳細的 AskUserQuestion 結構、9 種五官方向描述詞、參數覆核範本、5 段輸出格式時翻這份。
...
否則使用 SKILL.md §21 預設的 PROMPT + PARAMETERS 簡單格式。
...
如使用者問題已涵蓋風格（如「美背逆光」明顯指向 Backlit preset，或「領口鎖骨」指向 Neckline preset），跳過此題。
...
```text
[若使用者需求觸發紅色警示，則在最開頭顯示以下區塊，否則省略]
⚠️【安全警示與優化提示】
- ⚠️ 偵測到敏感或避審意圖詞彙：「濕比基尼、避審寫法」
- 💡 潛在風險：在 gpt-image-2 中極易觸發內容安全過濾導致生成失敗，或被解讀為低俗風格而產生質感廉價、油膩、塑膠感皮膚的畫面。
- ⭕ 安全高雅轉譯：已自動為您將其動態優化為綠色高端時尚與攝影專業語言，例如將「濕比基尼」轉譯為「dewy textured resort wear（露微濕質感度假服）」，將「避審」導向健康的時尚意圖。在確保 100% 順利通關的前提下，呈現頂級時尚雜誌寫真質感。
...
| 場景 | 用哪種輸出 |
|------|----------|
| 簡單請求（「給我一張寫真 prompt」）| §21 預設 PROMPT + PARAMETERS |
| 使用者貼完整參數表 | **5 段格式（Mode B）** |
| 使用者要求「精修」「商業寫真」 | **5 段格式（Mode B）** |
| 使用者要 API payload | §21 預設 + API payload |
| 使用者要 reference image edit | §23 範例格式 + API payload |
...
5. **5 段輸出格式忘了 §18.3 + §18.4 Constraints**：技術性防禦仍要嵌進去
