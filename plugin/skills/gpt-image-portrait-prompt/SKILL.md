...
> - `references/presets.md`：6 個大型風格 preset 與 3D CG 模式完整說明（§12–§17）
> - `references/examples.md`：五段式完整輸出範例（§22–§25）
> - `references/community-vetted/`：**Living Layer**（經 gate 的新有效 pattern，持續更新）
...
| 2 | 使用者貼**完整參數表**（風格 / 五官 / 場景 / 服裝 / 鏡頭 / 畫幅至少 4 項明示）| **直接 Mode B**：§3.3 參數覆核 + §21 5 段輸出，不問 |
| 3 | 使用者**明確指定五官方向**（清冷高級臉 / 東方丹鳳眼 / ...）**或**選 `身形：豐腴曲線` | **直接 Mode B**：同上 |
| 4 | **觸發 preset 詞**（美背 / 逆光 / 露背 / 夜色情緒 / 都市夜景 / 雨後街道 / 溫柔治癒 / 窗邊 / 古典東方 / 新中式 / 東方庭院 / 領口 / 頸部 / 鎖骨 / 3D CG / 幻想系 / 角色渲染 等），**但沒指定五官方向** | **啟動互動補完 + Mode B**：用 AskUserQuestion 至少問**五官方向**。觸發後請參考 `references/presets.md` 選擇對應 preset。**不可省略互動** |
...
4. 如為 Mode B 互動精修，輸出時用 §21 5 段格式呈現覆核結果
...
詳細黃色詞轉譯表見 `references/safety-glossary.md`。主體僅保留原則：單獨使用時需搭配中性/商業詞，避免與高風險場景組合。

**紅色高風險警示詞（不直接拒絕，自動啟動安全轉譯與優化提示）**：
...
完整轉譯表與詳細說明見 `references/safety-glossary.md`。Tier 1 嚴重違規仍絕對拒絕。

### 8.3 轉譯與引導原則

- 將低級慾望詞或避審偽裝詞，轉為服裝剪裁、姿態、光影、布料、輪廓、氣質與商業攝影語言，確保 100% 順利通關。
- 不主動使用「避審」「繞過限制」等語氣進行對抗，而是扮演專業安全轉譯顧問，將避審需求導向更高雅的視覺表達。
- 如果使用者意圖屬於嚴重違規紅線（露骨色情、未成年性感、真人私密深偽），必須執行 Tier 1 絕對拒絕，不得改寫包裝。
...
- elegant neckline design (cowl-neck, boat-neck, V-neck, high-collar)
- refined collarbone contour under side light
- tasteful fabric drape and structural lines
- composed upright or three-quarter posture
- natural neck and shoulder line

避免：

- deep cleavage focus or chest gap close-up
- loose unbuttoned shirt gaping open
- voyeuristic overhead POV looking down into neckline
- bending-forward postures designed to exaggerate exposure
- lingerie styling or pajama look

---

## 12–17. 可選風格 Preset 與視覺媒材模式

詳細內容（含 6 個大型 preset 的完整核心方向、推薦服裝、姿態、光線、避免事項，以及 3D CG 模式切換規則）已移至：

**`references/presets.md`**

**使用方式**：
- 當使用者觸發對應關鍵詞（美背、逆光、夜景、窗邊、新中式、領口、3D CG 等）但尚未指定五官方向時，**必須先啟動互動補完五官方向**。
- 互動完成後，請參考 `references/presets.md` 選擇對應 preset 進行細部描述。
- 主 SKILL.md 保留「何時觸發」與安全原則，完整指引移至 references 以降低主檔大小，提升跨 agent 相容性。

---

## 18. Prompt 生成方法

### 18.1 五段式結構（gpt-image-2 官方推薦）
...
**Constraints 是 GPT Image 系列唯一的「Negative Prompt」替代——必須寫，不能省。** 完整五段式輸出範例請見 `references/examples.md`。
...
### 18.3 防禦性排除指令（取代 Negative Prompt）
...
9. **Constraints**：完整 §18.3 + §18.4 防禦組
...
9. Safety / exclusion constraints（§18.3 四層 + §18.4 物理瑕疵）
...
**重要：`gemini-omni-flash` 不是純圖像生成模型**（主功能為 video，超出本 skill 範疇）。

跨模型相容性重點請見 `references/api-reference.md`。主體原則：
- gpt-image-2 預設用五段式 + inline Constraints。
- Gemini 系列偏好 narrative paragraph + 正向重寫。
- 使用者指名其他模型時，依 `references/api-reference.md` 調整結構。
...
完整穩定尺寸對照表、4K 注意事項與成本控制策略，請見 `references/api-reference.md`。

主體原則：gpt-image-2 兩邊必須為 16 倍數；Gemini 用 tier 制；使用者未指定 4K 時優先用 2K 或以下。
...
### ⚠️【安全警示與優化提示】輸出規則
當使用者的輸入需求觸發了紅色高風險警示詞（如「性感」、「濕身」、「避審」、「限界突破」等黃線/避審範圍）時，無論是 Mode A 還是 Mode B，**都必須在輸出最開頭（PROMPT 或 參數覆核 之前）**加上以下結構：

```text
⚠️【安全警示與優化提示】
- ⚠️ 偵測到敏感或避審意圖詞彙：「[使用者輸入的敏感詞]」
- 💡 潛在風險：在 gpt-image-2 中極易觸發內容安全審查導致生成失敗，或被模型解讀為低俗風格而產生質感廉價、油膩、塑膠感皮膚的畫面。
- ⭕ 安全高雅轉譯：已自動為您將其動態優化為綠色高尚時尚與攝影專業語言，例如將「[敏感詞]」轉譯為「[英文轉譯詞]」（如 dewy textured resort wear、tasteful open-back cut 等）。在 100% 確保合規通關的前提下，呈現極致高端的時尚雜誌寫真質感。
```
...
- 參數覆核（依 §3.3 強制）
- §18.3 四層防禦 Constraints（仍嵌入 prompt 內）
- §18.4 物理瑕疵負面詞
- 模型相容性調整（依 §20，Gemini 用 narrative paragraph + 正向重寫 constraints）
...
詳細的四個完整輸出範例（預設寫真、Reference Image 一致性、Backlit 美背、3D CG 模式）已移至：

**`references/examples.md`**

**使用方式**：
- 這些範例的主要目的是展示「五段式結構 + 完整 Constraints」的正確語氣與安全寫法。
- 實際生成時，請根據 §18 的結構 + 當前使用者需求 + `references/presets.md` 重新組裝，不要直接複製範例。
- 主 SKILL.md 保留使用時機與原則，完整 few-shot 移至 references 以降低主檔大小。

---

## 26. 警示與安全優化策略

當觸發敏感或避審傾向詞彙時，採用**雙層合規防禦架構**：
- **Tier 1（嚴重違規）**：裸露、未成年性感化、真人深偽 → **絕對拒絕** + 提供健康替代方向。
- **Tier 2（黃線）**：一般敏感詞、避審語 → **不拒絕**，輸出 `⚠️【安全警示與優化提示】` 並自動轉譯為高端綠色攝影語言。

完整雙層架構說明與替代方向請見 `references/safety-glossary.md`。
...
- [ ] 是否把「性感」轉成克制、高級、攝影語言？
- [ ] 是否避免身體部位特寫？
- [ ] 是否避免高風險組合詞（可愛 + 性感、學生 + 性感、床上 + 衣物滑落，除非已進行 Tier 2 安全轉譯）？
- [ ] 是否使用英文最終 prompt？
- [ ] 是否依五段式結構（Scene / Subject / Details / Lighting / Use case / Constraints）？
- [ ] Constraints 段是否包含 §18.3 四層防禦 + §18.4 物理瑕疵？
...
本 Skill 對使用者的合規管理聲明如下：

1. **拒絕對抗性避審，擁抱安全美學轉譯**：
   我們不參與生硬的「避審對抗」或「Jailbreak 繞過」。當使用者請求包含「限界突破」「迴避策」「避審」「繞過 OpenAI 限制」等意圖或詞彙時，我們**不再生硬拒絕**一般請求，而是扮演**專業安全轉譯顧問**。我們溫和提示使用者繞過行為的生圖失敗風險，並**自動且直接將其轉譯為 100% 合規且質感更高雅的綠色攝影與時尚語彙**。

2. **欺騙性親密偽裝之安全引導**：
   對於試圖利用「藝術/舞蹈/雕塑/秘密」等詞彙包裝的多人親密接觸或性化意圖（如「彫刻のように抱き合う」、「情熱的なダンス的密著」），我們同樣**不進行生硬回絕**，而是將其**動態轉譯為完全合規、健康的單人高品味時尚人像表達**（如單人優雅的雕塑般舒展姿態 `sculptural beauty of a poised silhouette`），並附上安全警示。

3. **嚴重違規硬性拒絕（Tier 1）**：
   任何涉及未成年性感化、真實裸露與色情、真人私密肖像深偽的請求，屬於嚴重違反 API 服務條款與社會道德之行為。此類請求**不予轉譯，必須直接拒絕**並提供健康合規方向。

本 Skill 的核心價值在於**幫使用者把意圖動態優化並翻譯成模型聽得懂的合規高端視覺語言**，而不是**幫使用者藏意圖騙過審查**。透過雙層合規防禦，我們既保護了系統合規安全，又給予使用者最高的創作成功率與畫面美感。
