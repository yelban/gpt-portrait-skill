...
| 情境 | 行為 |
|------|------|
| 已提供足夠條件 | 直接執行 §3.3 參數覆核 + 輸出 prompt |
| 缺少重要參數 | 啟動互動補完，最多一輪、最多 4 題 |
| 使用者說「自動 / 你決定 / 直接生成 / 幫我配 / 不用問 / 給我一版」 | 不追問，用 §5 預設值 |
| 使用者貼完整參數表 / 要求精修 / 商業寫真 / 高級人像 | **進入互動精修模式（Mode B）**，見 `references/interactive-templates.md` |
| 使用者明確指定五官方向 / 豐腴曲線 | **同上 Mode B**，必含參數覆核 + 5 段輸出 |
